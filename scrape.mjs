...
// 아예 손대지 않는다(새 기록 추가는 물론, 마지막 기록의 시각/원화값도 갱신하지 않음).
// 예전엔 값이 안 바뀌어도 마지막 기록의 "시각"만은 매 실행마다 지금 시각으로 계속 갱신했는데,
// 그러면 실제로는 안 움직인 카드들의 "마지막 기록 시각"이 매 스크래핑 실행마다 전부 같은 시각으로
// 동기화되어 버려서, 포트폴리오 히스토리 집계에서 마치 그 시점에 여러 카드가 한꺼번에 변동한 것처럼
// 보이는 부작용이 있었다(환율이 살짝만 움직여도 krw가 재계산되면서 "변동"으로 잡혔음). 값이 그대로면
// 기록을 아예 건드리지 않아야 이 문제가 원천적으로 사라진다.
function appendHistory(card, now, krw, jpy) {
  ...
  if (!last || last[2] !== jpy) {
    // 엔화 원가가 지난 기록과 다를 때만 새 (시각, 원화가, 엔화원가) 기록을 추가한다.
